...
github_url = 'https://api.github.com/repos/BaritoLog/BaritoMarket/releases/latest'


puts "App Name: #{app_name}"
puts "Gate Script: #{market_script_location}"
...
release_file = JSON.parse(Chef::HTTP.new(github_url).get(""))["assets"][0]["browser_download_url"]
release_name = JSON.parse(Chef::HTTP.new(github_url).get(''))['tag_name']
...
execute "make source directory" do
  ...
  group app_name
  action :run
  not_if { ::File.exist?("/opt/#{app_name}/#{release_name}") } 
end
...
template "/etc/systemd/system/puma.service" do
  ...
  group app_name
  mode "00644"
  variables( app_name: app_name, app_home: app_name, market_script_location: market_script_location )
